...
import habitacionService from "./../../service/habitacionService.js"
...
async function getTipoHabitaciones() {
    const { data } = await habitacionService.listar();
...
async function guardar() {
    if (tipohabitacion.value._id) {
        ...
        try {
            ...
            await habitacionService.modificar(tipohabitacion.value._id, tipohabitacion.value)
        ...
        try {
            ...
            await habitacionService.guardar(tipohabitacion.value)
...
async function confirmDeleteTipoHabitacion(th) {
    if (confirm("¿Está seguro de eliminar?")) {
        await habitacionService.eliminar(th._id);
...
<template>
    <div class="card">
        <!--<Button label="Nueva Habitación" @click="visible = true" /> -->

        <DataTable :value="tipo_habitaciones" tableStyle="min-width: 50rem">
            <Column field="tipo_habitacion_id.nombre" header="Tipo habitación"></Column>
            <Column field="disponible" header="Disponibilidad"></Column>
            <Column field="numero" header="Nro Hab."></Column>
            <Column field="precio" header="Precio Base (Bs)"></Column>
            <Column field="comodidades" header="Comodidades"></Column>
            <Column field="estado_mantenimiento" header="Estado"></Column>
